...
cd RasPike-ART
```
recursiveオプションをつけるのを忘れないようにしてください。
...
# USBの接続がうまく行かなくなった場合
RasPi側のプログラムを起動した際、USBが認識されずにエラーで止まる場合があります。これはRasPiのUSB周りの不具合と思われますが、今のところ原因はわかっていません。
RasPi側のプログラムを停止(Ctl+c)せずに先にSPIKE側の電源を切った場合に発生するケースが多いですが、絶対ではありません。
この場合、対処方法としてはRasPi側のUSB-Aポートを別のポートに切り替えるか、RasPiを再起動する必要があります。
原因などがわかった方はぜひお知らせください。
